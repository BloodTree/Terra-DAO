...
module.exports = async function (deployer, network, accounts) {
  ...
  await deployer.deploy(_TimeBondingCalculator, TimeERC20Token.address);
  const TimeBondingCalculator = await _TimeBondingCalculator.deployed();
  await deployer.deploy(_Distributor, TimeTreasury.address, TimeERC20Token.address, epochLength, nextEpochBlock);
